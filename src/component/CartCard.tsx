...
const CartCard: FC<CartCardProps> = ({ cartList }) => {
    const newCart = _.uniq(cartList);
    newCart.forEach((el) => { 
        el.count = 0;
        const filtred = cartList.filter((p) => (p.title === el.title && p.img === el.img))
        el.count = filtred.length
    })
    const a = _.uniq(newCart.map((el) => JSON.stringify(el))).map((el) => JSON.parse(el))


    return (
    <div className="productsCart">
        {a.length === 0 ? <div className="emptyCart">Корзина пуста</div> : a.map((el) => {
            return (
                <div className="productCart">
                    <div className="picAndCount">
                        <img src={el.img} alt={el.title}/>
                        <div className="counts">
                            <button className="countsBtn" onClick={() => {alert('я не могу сообразить как заставить эти кнопки работать :(')}}>
                    ...
                    <div className="delete">
                        ...
                        <p>{el.count ? el.count * el.price : ''} ₽</p>
                    </div>
                </div>
            )
        })}
